...
const Gradebook = () => {
    ...
    const calculateStats = (studentId) => {
        ...
        const findGrade = (type) => {
            ...
            return g ? (g.marksObtained || g.MarksObtained || 0) : null;
        };

        const carryMark = findGrade(6);     // Type 6
        const clearanceMark = findGrade(5); // Type 5
        const finalMark = findGrade(3);     // Type 3
        const attdScore = findGrade(0) || 0; // Type 0
        ...
        let finalTotal = 0;
        let displayExamScore = 0;

        // PRIORITY LOGIC: Carry > Clearance > Regular
        if (carryMark !== null) {
            finalTotal = (carryMark / 70) * 100;
            displayExamScore = carryMark;
        } else if (clearanceMark !== null) {
            finalTotal = caTotal + clearanceMark;
            displayExamScore = clearanceMark;
        } else {
            finalTotal = caTotal + (finalMark || 0);
            displayExamScore = finalMark || 0;
        }

        return {
            attendance: attdScore.toFixed(2),
            quizzes: quizScore.toFixed(2),
            final: displayExamScore.toFixed(2),
            total: Math.min(finalTotal, 100).toFixed(2)
        };
    };
                            ...
                            {(data.enrollments || []).map(e => (
                                <tr key={e.studentId}>
                                    ...
                                    {(data.assessments || []).map(a => {
                                        ...
                                        return (
                                            <td key={a.id || a.Id}>
                                                {/* UPDATED: Validation logic for Max Marks (Step 2) */}
                                                <input
                                                    type="number"
                                                    ...
                                                    onBlur={ev => {
                                                        ...
                                                        if (val > max) {
                                                            alert(`Error: Mark (${val}) cannot exceed Max Marks (${max}) for ${a.title || a.Title}`);
                                                            ev.target.value = g ? (g.marksObtained || g.MarksObtained) : '';
                                                            return;
                                                        }
            ...
            {showAddModal && (
                <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    <div className="user-info-card" style={{ width: '400px' }}>
                        ...
                        {/* UPDATED: Added Clearance & Carry (Step 1) */}
                        ...
                        <input type="number" value={newCol.maxMarks} className="form-input" style={{ width: '100%', marginBottom: 15 }} placeholder="20" onChange={e => setNewCol({ ...newCol, maxMarks: parseFloat(e.target.value) })} />
